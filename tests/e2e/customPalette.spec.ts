...
test('custom palette adds, plays, persists, deduplicates and removes chords', async ({
  ...
  await dialog.getByRole('button', { name: 'CM9', exact: true }).click();
  await dialog
  ...
  const added = outside.getByRole('button', { name: /^CM9 / });
  await expect(added).toBeEnabled();
  await added.click();
  await expect(page.getByTestId('chord-symbol')).toHaveText('CM9');
  await expect
    .poll(async () =>
  ...
  await open.click();
  await dialog.getByRole('button', { name: 'CM9 削除', exact: true }).click();
