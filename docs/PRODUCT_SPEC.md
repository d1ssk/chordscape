...
C/Dのような非構成音bassはslash chordとして別データで表し、「第n転回」と呼ばない。コード編集の「ベース音」で任意の音名を指定すると、上の和音の配置を保ってその下に1音を追加する。指定変更は追加した1音だけを置き換え、「指定なし」で取り除く。構成音と同じ音名でも独立した低音として追加し、最大8音になる。

指定中の転回形操作は「上の和音の配置」として働く。追加ベースが引き続き最低音になるよう必要ならその音だけオクターブを下げる。±1 octaveはベースを含む全体を動かす。上の和音はMIDI 36〜96、追加ベースは24以上で上の和音より低く配置する。Root / Smoothでもベースの音名・綴りを保持し、手動再生は保存した実音をそのまま使う。

`ChordEvent.addedBass` に独立ベースの綴りを持ち、`bass` は上の和音の転回指定、`notes` は追加ベースを含む昇順の実音とする。表示・鍵盤・発音・移調・Undo・JSONは同じイベントに従う。追加ベース指定時のRomanや機能説明は「上の和音」の分析として示し、和音名を別のqualityへ自動変換しない。旋律生成も上の和音を基準とする。保存はschemaVersion 5 / `chordscape.session.v5`、既存v1〜v4を読み込んで移行する。
