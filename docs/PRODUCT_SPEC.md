...
既定候補に加え、「＋ 追加」から根音と17種類のqualityを選んで任意の和音を登録できる。追加分は個別に削除可能。同じ綴り・qualityの重複表示を防ぎ、調を変えても登録したコード名を保持する。追加分はこのブラウザのlocalStorageへ即時保存し、進行のJSONとは独立させる。保存不可でも画面内では使用でき、警告を表示する。
